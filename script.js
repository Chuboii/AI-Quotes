...
setTimeout(() => {
  ...
  /* generating programming jokes */

  let proJokesLink = document.querySelector(".pro-jokes")
  let proPage = document.querySelector(".pro-page-container")
  const delay = 1000
  let timer = 7
  let intervals
  proJokesLink.addEventListener("click", () => {
    ...
    async function proJokes() {
      try{
      let line = document.querySelector(".line-pro")
let jokeSetup = document.querySelector(".jokes-setup-pro")
let time = document.querySelector(".time-pro")
let nextBtn = document.querySelector(".next-pro")
let copyBtn = document.querySelector(".copy-pro")
let prevBtn = document.querySelector(".prev-pro")
      let cateText = document.querySelector(".cate-text-pro")
let punchlineText = document.querySelector(".punchline")
      ...
      const punchline = data[0].punchline
        jokeSetup.innerHTML = ques
      punchlineText.innerHTML = punchline
      cateText.innerHTML = "Programming jokes"
      cateText.style.fontSize = "10px"
      ...
      intervals = setInterval(() => {
        timer--
        if (timer >= 0) {
          line.style.width = `${100 - (timer * (100 / 7))}%`
          time.innerHTML = `00:0${timer}`
        }
        else {
  timer = 7
...
async function nextProJokes(){
  try{
    const res2 = await fetch("https://backend-omega-seven.vercel.app/api/getjoke")
    
      const data2 = await res2.json()
      let ques2 = data2[0].question
    let punchline2 = data2[0].punchline
    
      jokeSetup.innerHTML = ques2
      punchlineText.innerHTML = punchline2
    ...
    console.log(e)
     ...
     async function nextBtnProJoke(){

       let res3 = await fetch("https://backend-omega-seven.vercel.app/api/getjoke")

      let data3 = await res3.json()
      let ques3 = data3[0].question
      let punchline3 = data3[0].punchline
      
      jokeSetup.innerText = ques3
      punchlineText.innerText = punchline3
      
       arrDataPrev3.quess = ques
       arrDataPrev3.punchlin = punchline3
  ...
  time.innerHTML = `00:0${timer++}`
    ...
    prevBtn.addEventListener("click", () => {
      let storedQuoteLength = arrDataPrev3.length
      storedQuoteLength--
      if(storedQuoteLength >= 0){
        jokeSetup.innerText = arrDataPrev3[storedQuoteLength].quess
        punchlineText.innerHTML = arrDataPrev3[storedQuoteLength].punchlin
